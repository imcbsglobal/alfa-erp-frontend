  ...
  const loadSessions = async () => {
    ...
    try {
      ...
      if (deliveryTypeFilter !== 'ALL') {
        params.delivery_type = deliveryTypeFilter;
      }

      const applyLocalFilters = (rows) => {
        let out = [...rows];
        if (statusFilter !== 'ALL') {
          out = out.filter((s) => (s.delivery_status || '') === statusFilter);
        }
        if (deliveryTypeFilter !== 'ALL') {
          out = out.filter((s) => (s.delivery_type || '') === deliveryTypeFilter);
        }
        return out;
      };
      ...
      const searchedResults = !q
        ...
        : allResults.filter((s) => [
          ...
          s.customer_area,
          s.customer_address,
          s.delivery_user_name,
          s.delivery_user_email,
          s.courier_name,
          s.tracking_no,
          s.notes,
        ]
