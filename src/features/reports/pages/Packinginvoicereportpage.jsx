...
export default function PackingInvoiceReportPage() {
  ...
  const [packerQuery, setPackerQuery] = useState('');
  const [timeFilter, setTimeFilter] = useState('');
  const invoiceSearchRef = useRef(null);

  const debouncedSearch = useDebounce(searchQuery, 500);
  const debouncedPacker = useDebounce(packerQuery, 300);
  ...
  useEffect(() => {
    if (!debouncedPacker.trim()) {
      setSessions(allSessions);
    } else {
      const q = debouncedPacker.toLowerCase();
      setSessions(allSessions.filter(s => (s.packer_name || '').toLowerCase().includes(q)));
    }
  }, [debouncedPacker, allSessions]);
  ...
  const loadSessions = async () => {
    ...
    try {
      ...
      if (debouncedPacker.trim()) {
        const q = debouncedPacker.toLowerCase();
        setSessions(results.filter(s => (s.packer_name || '').toLowerCase().includes(q)));
      } else {
        setSessions(results);
      }
    } catch (err) {
  ...
  const handleViewSession = (session) => {
    const invoiceData = {
      ...
      packer_info: session.packer_name ? { name: session.packer_name, email: session.packer_email, end_time: session.end_time } : null,
      delivery_info: null,
    };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-sm p-3 mb-4">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-1.5">
              <label className="text-sm font-semibold text-gray-600 whitespace-nowrap">Packer:</label>
              <div className="relative">
                <Search size={13} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
                <input
                  type="text"
                  ...
                  value={packerQuery}
                  onChange={(e) => setPackerQuery(e.target.value)}
                  className="pl-7 pr-7 py-1.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 text-sm w-[245px]"
                />
                {packerQuery && (
                  <button onClick={() => setPackerQuery('')}
                    className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                    <X size={13} />
                  </button>
                )}
